...
class TyphoonSelectDialog2: NSObject, UITableViewDelegate, UITableViewDataSource {
    ...
    init(index: Int, parentView: TyphoonViewController){
        ...
        switch index {
        case 1:
            items = ["■氾濫注意水位(水位4.5m)、水防警報(出動)", "■避難準備・高齢者等避難開始発令の見込み(1時間以内に水位5.4mに到達)", "■【警戒レベル３】避難準備・高齢者等避難開始(水位5.4m)、避難勧告発令の見込み", "■【警戒レベル４】避難勧告(水位5.5m)", "■【警戒レベル４】避難指示(緊急)(水位8.3m)"] //淀川（枚方）
            break
        case 2:
    ...
    func showInfo (){
        ...
        win1.frame = CGRect(x: 80,y: 200,width: parent.view.frame.width-40,height: parent.view.frame.height*0.8)
